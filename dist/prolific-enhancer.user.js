...
(() => {
  ...
  var BaseAdapter = class {
    ...
    siteName;
    settings;
    constructor(url, defaults, overrides = {}) {
      this.url = url;
  ...
  // src/lib/getSiteAdapter.ts
  var siteAdapters = [new ProlificAdapter(), new CloudResearchAdapter()];
  function matchesHost(host, allowedHost) {
  ...
  var fetchResources_default = fetchResources;

  // src/lib/getSiteResources.ts
  var adapter = getSiteAdapter_default();
  var getSharedResources = fetchResources_default(adapter.siteName);
  var getSiteResources_default = getSharedResources;
  ...
  var NewSurveyNotificationsEnhancement = class extends BaseEnhancement {
    async apply() {
      ...
      const assets = await getSiteResources_default();
  ...
  function rateToColor(rate, min = 7, max = 15) {
    ...
    const r2 = Math.round(255 * (1 - bias));
    const g = Math.round(255 * bias);
    return `rgba(${r2}, ${g}, 0, 0.63)`;
  }
  ...
  var HighlightRatesEnhancement = class extends BaseEnhancement {
    async apply() {
      ...
      for (const element of elements) {
        ...
        const { displaySymbol, sourceSymbol } = this.adapter.getCurrencyInfo(element);
